...
const CustomInput = forwardRef(
  (
    ...
    const variantStyles = {
      ...
      searchBox:
        "px-8 py-3 text-custom-ghostWhite bg-grey-19 max-h-12 rounded-containers border border-hover-grey-1",
      apiComponent:
        "bg-transparent text-custom-ghostWhite h-[40px] px-[8px] rounded-[5px] max-h-12 border border-gradient-grey-4 w-full leading-none",
    };
    ...
    return (
      <div className={`${containerBaseStyles} ${className}`}>
        {(label || endLabel) && (
          <div className="flex justify-between mb-2 text-grey-17">
            <p className={`font-medium leading-11 h-max ${labelClassName}`}>
              {label}
            </p>
            <Button className={`font-normal leading-base ${endLabelClassName}`}>
              {endLabel}
            </Button>
          </div>
        )}
        <div className="relative">
          {(prefix || variant === "searchBox") && variant === "searchBox" ? (
            <SearchIcon
              className={`absolute top-1/2 left-3 -translate-y-1/2 ${SearchIconClassName || "icon-white"}  `}
            />
          ) : (
            <span
              ref={prefixRef}
