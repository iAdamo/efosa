...
function OAuthForm({ handleAuthClick, authData, setAuthData, handleBasicAuthCheck, isLoading, validationIssues }) {
    ...
    const [inputClientSecretFieldValue, setInputClientSecretFieldValue] = useState(
        '',
    );
            ...
            <div className='flex flex-col gap-6'>
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <span className='text-grey-17 font-medium text-base'>{isEditingUserName ? "EDIT LABEL" : authData.oauth.clientIDFieldName || "Client ID"}</span>
                        {isEditingUserName ? (
                            <div className='flex gap-[7px]'>
                                <Button
                                    ...
                                    className="p-0"
                                >
                                    <span className='text-grey-17 hover:text-custom-ghostWhite underline underline-offset-4 leading-[14px]'>Save</span>
                                </Button>
                                <Button
                            ...
                            <Button
                                onClick={() => {
                                    ...
                                    setNewClientIDFieldName(authData.oauth.clientIDFieldName)
                                }}
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <span className='text-grey-17 font-medium text-base'>{isEditingPassword ? "EDIT LABEL" : authData.oauth.clientSecretFieldName || "Client Secret"}</span>
                        {isEditingPassword ? (
                            <div className='flex gap-[7px]'>
                                <Button
                                    onClick={
                            ...
                            <Button
                                onClick={() => {
                                    ...
                                    setNewClientSecretFieldName(authData?.oauth.clientSecretFieldName)
                                }}
                                className="p-0 text-grey-17 hover:text-custom-ghostWhite underline underline-offset-4 leading-[14px]"
                            >
                                Edit label
                    ...
                    {isEditingPassword ?
                        <CustomInput
                            ...
                            suffix={
                                <ToggleButtonGroupCustom
                                    name={"clientSecret"}
                                    alignment={authData?.oauth?.clientIDPlacement?.toLowerCase() || alignment?.["clientSecret"]}
                                    options={[
                                        { value: 'body', label: 'Body' },
                                        { value: 'header', label: 'Header' },
                                    ]}
                        ...
                        : <CustomInput
                            ...
                            onChange={(e) => setInputClientSecretFieldValue(e.target.value)}
                            suffix={
                                <ToggleButtonGroupCustom
                                    name={"clientSecret"}
                ...
                {/* Custom row */}
                <AddAnotherField authData={authData} setAuthData={setAuthData} handleToggleChange={handleChange} handleBasicAuthCheck={handleBasicAuthCheck} />
            </div>


            {validationIssues && Object.keys(validationIssues?.errors).length > 0 &&
                <div>
                    <ValidationErrorBox errors={validationIssues?.errors} />
                    <Button onClick={handleAuthClick} variant={'addAuth'} className='w-[104px] mt-4'>Try again</Button>
                </div>
            }
            {!validationIssues && <div className='flex gap-3'>
                {(isLoading) ? <CustomLoader iconClassName='icon-pink-1' labelClassName='text-secondary-pink-light-1' label='Authenticating your API....' /> :
                    <>
                        <Button
