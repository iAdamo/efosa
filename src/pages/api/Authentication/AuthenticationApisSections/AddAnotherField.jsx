import { deleteGenericCRUDWithID, postGenericCRUD, postGenericCRUDWithID } from '@/axios/apiCalls';
...
function AddAnotherField({ authData, setAuthData, handleToggleChange, handleBasicAuthCheck }) {
                ...
                <div key={customRow?.id}>
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-grey-17 font-medium text-base">
                            {editingRows[customRow.id] ? "EDIT LABEL" : customRow?.fieldName || "NEW ROW"}
                        </span>
                        ...
                        {editingRows[customRow.id] ? (
                            ...
                            <div className='flex items-center gap-[13px]'>
                                ...
                                <Button className='p-0' onClick={async () => {
                                    try {
                                        await deleteGenericCRUDWithID("Authentication_OAuth_Custom", customRow?.id);

                                        setAuthData((prevAuthData) => {
                                            const updatedRows = prevAuthData.oauth.customRows.filter(
                    ...
                    {editingRows[customRow.id] ? (
                        <CustomInput
                            ...
                            onChange={(e) => handleFieldNameChange(customRow.id, e.target.value)}
                        ...
                        <CustomInput
                            ...
                            onChange={(e) => handleFieldValueChange(customRow.id, e.target.value)}
                            suffix={
                                <ToggleButtonGroupCustom
                                    name={`customField-${customRow.id}`}
                                    alignment={
                                        authData?.oauth?.customRows[index]?.bodyOrHeader?.toLowerCase() ||
                                        alignment?.[`customField-${customRow?.id}`]
                                    }
                                    handleChange={(event, newAlignment) =>
                                        handleToggleChange(event, newAlignment, `customField-${customRow?.id}`, customRow?.id, index)
                                    }
                                    options={[
